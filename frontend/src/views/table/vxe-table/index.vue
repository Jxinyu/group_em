...
import { deleteTableDataApi, getTableDocumentDataApi } from "@/api/table"
    ...
    ajax: {
      query: ({ page, form }: VxeGridPropTypes.ProxyAjaxQueryParams) => {
        ...
        return new Promise<any>((resolve: Function) => {
          ...
          /** 接口需要的参数 */
          const params = {
            username: form.loginname || undefined,
            phone: form.phone || undefined,
            size: page.pageSize,
            currentPage: page.currentPage
          }
          /** 调用接口 */
          getTableDocumentDataApi(params).then(callback).catch(callback)
        })
      }
    }
  }
...
const crudStore = reactive({
  ...
  onDelete: (row: RowMeta) => {
    const tip = `确定 <strong style='color:red;'>删除</strong> 用户 <strong style='color:#409eff;'>${row.loginname}</strong> ？`
